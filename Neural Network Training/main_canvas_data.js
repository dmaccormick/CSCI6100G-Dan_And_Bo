...
function setup() {
    ...
    for (let i = 0; i < 3; i++)
    {
        ...
        image(imageList[i], 0, 0);

        const imageWidth = imageList[i].width;
        const imageHeight = imageList[i].height;

        canvasObject.width = 150.0; //imageWidth;
        canvasObject.height = 100.0; //imageHeight;

        //canvasObject.getContext('2d').drawImage(imageList[i], 0, 0, 150.0, 100.0);

        // get the pixel data back from the canvas
        let pixelData = canvasObject.getContext('2d').getImageData(0, 0, 150, 100);
    ...
    console.log(imageListNormalized);
